...
Description: Scrapes talk data and scripture references from the April 2026 General Conference 
...
def webscraping():
  ...
  # We do this so that if we run the script multiple times, we don't just keep adding the exact same talks to the database over and over (creating duplicates).
  ...
  for link in talk_links:
      ...
      if 'session' in href.lower() or '/study/general-conference/2026/04' not in href:
          continue
